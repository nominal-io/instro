...
class BK914X(PSUDriverBase):
    ...
    def set_overvoltage_protection_enabled(self, enabled: bool, channel: int = 1) -> None:
        raise FeatureNotSupportedError(
            "set_overvoltage_protection_enabled is not supported by BK914X; "
            "the 9140 Series programming manual, section 17.1 Output Configuration, states that over voltage "
            "protection cannot be disabled."
        )

    def get_overvoltage_protection_enabled(self, channel: int = 1) -> bool:
        raise FeatureNotSupportedError(
            "get_overvoltage_protection_enabled is not supported by BK914X; "
            "the 9140 Series programming manual exposes OVP level and tripped queries, but no OVP enable-state query."
        )

    def set_overvoltage_protection_delay(self, delay: float, channel: int = 1) -> None:
        raise FeatureNotSupportedError(
            "set_overvoltage_protection_delay is not supported by BK914X; "
            "the 9140 Series programming manual does not define an OVP delay command."
        )

    def get_overvoltage_protection_delay(self, channel: int = 1) -> float:
        raise FeatureNotSupportedError(
            "get_overvoltage_protection_delay is not supported by BK914X; "
            "the 9140 Series programming manual does not define an OVP delay query."
        )
