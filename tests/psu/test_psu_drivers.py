...
def test_sim_set_voltage_includes_channel_prefix(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
...
def test_sim_get_voltage_includes_channel_prefix(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
...
def test_sim_set_current_limit_writes_source_current(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_current_limit(0.75, channel=2)
    sim_visa.write.assert_called_once_with(":SOUR2:CURR 0.750")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_get_current_includes_channel_prefix(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["0.123", '0,"No error"']
    assert sim.get_current(channel=2) == pytest.approx(0.123)
    assert sim_visa.query.call_args_list == [call(":MEAS2:CURR?"), call(":SYST:ERR?")]


def test_sim_output_enable_includes_channel_prefix(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.output_enable(True, channel=2)
    sim_visa.write.assert_called_once_with(":OUTP2:STAT ON")
    sim.output_enable(False, channel=2)
    assert sim_visa.write.call_args_list[-1] == call(":OUTP2:STAT OFF")
...
def test_sim_set_overvoltage_protection_level_writes_level(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_overvoltage_protection_level(12.5, channel=2)
    sim_visa.write.assert_called_once_with(":SOUR2:VOLT:PROT 12.500")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_get_overvoltage_protection_level_queries_level(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["12.5", '0,"No error"']
    assert sim.get_overvoltage_protection_level(channel=2) == pytest.approx(12.5)
    assert sim_visa.query.call_args_list == [call(":SOUR2:VOLT:PROT:LEV?"), call(":SYST:ERR?")]


def test_sim_set_overvoltage_protection_enabled_writes_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_overvoltage_protection_enabled(True, channel=2)
    sim_visa.write.assert_called_once_with(":SOUR2:VOLT:PROT:STAT ON")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_disable_overvoltage_protection_writes_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_overvoltage_protection_enabled(False, channel=2)
    sim_visa.write.assert_called_once_with(":SOUR2:VOLT:PROT:STAT OFF")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_get_overvoltage_protection_enabled_parses_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["1", '0,"No error"']
    assert sim.get_overvoltage_protection_enabled(channel=2) is True
    assert sim_visa.query.call_args_list == [call(":SOUR2:VOLT:PROT:STAT?"), call(":SYST:ERR?")]


def test_sim_get_overvoltage_protection_enabled_parses_disabled(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["0", '0,"No error"']
    assert sim.get_overvoltage_protection_enabled(channel=2) is False
    assert sim_visa.query.call_args_list == [call(":SOUR2:VOLT:PROT:STAT?"), call(":SYST:ERR?")]


def test_sim_set_overcurrent_protection_level_writes_level(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_overcurrent_protection_level(0.8, channel=2)
    sim_visa.write.assert_called_once_with(":SOUR2:CURR:PROT 0.800")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_get_overcurrent_protection_level_queries_level(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["0.8", '0,"No error"']
    assert sim.get_overcurrent_protection_level(channel=2) == pytest.approx(0.8)
    assert sim_visa.query.call_args_list == [call(":SOUR2:CURR:PROT:LEV?"), call(":SYST:ERR?")]


def test_sim_set_overcurrent_protection_enabled_writes_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_overcurrent_protection_enabled(True, channel=2)
    sim_visa.write.assert_called_once_with(":SOUR2:CURR:PROT:STAT ON")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_disable_overcurrent_protection_writes_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_overcurrent_protection_enabled(False, channel=2)
    sim_visa.write.assert_called_once_with(":SOUR2:CURR:PROT:STAT OFF")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_get_overcurrent_protection_enabled_parses_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["1", '0,"No error"']
    assert sim.get_overcurrent_protection_enabled(channel=2) is True
    assert sim_visa.query.call_args_list == [call(":SOUR2:CURR:PROT:STAT?"), call(":SYST:ERR?")]


def test_sim_get_overcurrent_protection_enabled_parses_disabled(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["0", '0,"No error"']
    assert sim.get_overcurrent_protection_enabled(channel=2) is False
    assert sim_visa.query.call_args_list == [call(":SOUR2:CURR:PROT:STAT?"), call(":SYST:ERR?")]


def test_sim_set_remote_sense_enabled_writes_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_remote_sense_enabled(True, channel=2)
    sim_visa.write.assert_called_once_with(":SYST2:SENS REM")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_disable_remote_sense_writes_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim.set_remote_sense_enabled(False, channel=2)
    sim_visa.write.assert_called_once_with(":SYST2:SENS LOC")
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_get_remote_sense_enabled_parses_state(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["REM", '0,"No error"']
    assert sim.get_remote_sense_enabled(channel=2) is True
    assert sim_visa.query.call_args_list == [call(":SYST2:SENS?"), call(":SYST:ERR?")]


def test_sim_get_remote_sense_enabled_parses_local(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.side_effect = ["LOC", '0,"No error"']
    assert sim.get_remote_sense_enabled(channel=2) is False
    assert sim_visa.query.call_args_list == [call(":SYST2:SENS?"), call(":SYST:ERR?")]


def test_sim_check_errors_accepts_plus_zero(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
    sim_visa.query.return_value = '+0,"No error"'
    sim.set_voltage(1.0)
    sim_visa.query.assert_called_once_with(":SYST:ERR?")


def test_sim_check_errors_raises_runtime_error(sim: SimulatedPSU, sim_visa: MagicMock) -> None:
