"""Closing Bell - Step 3: modulation, burst, and sweep on the Keysight 33521B."""
...
def _apply_settled(fn, *args) -> None:
    """Retries a call whose target state is already correct so a queued, self-resolving
    Keysight -221 advisory gets drained on a repeat (no-op) write instead of raising."""
...
try:
    # ------------- Modulation tests ----------------
    _apply_settled(awg.set_waveform, CHANNEL, Pulse(frequency_hz=10_000.0, width_s=50e-6))
    awg.output_enable(CHANNEL, True)

    _apply_settled(awg.set_modulation, CHANNEL, ModulationType.PWM, Sine(frequency_hz=100.0), 5e-6)
    ...
    print("mod still enabled:", awg.get_modulation_state(CHANNEL).latest)
    time.sleep(WAIT_TIME)
    ...
    awg.set_burst_trigger(CHANNEL, BurstTriggerSource.MANUAL)
    print("burst type:", awg.get_burst_type(CHANNEL))
    print("ncycles (INFINITE sentinel):", awg.get_burst_ncycles(CHANNEL).latest)
    awg.fire_burst_trigger(CHANNEL)
    time.sleep(WAIT_TIME)

    awg.set_burst(CHANNEL, BurstType.GATED)
    print("burst type:", awg.get_burst_type(CHANNEL))
    awg.set_burst_trigger(CHANNEL, BurstTriggerSource.EXTERNAL)
    print("trigger accepted while GATED:", awg.get_burst_trigger(CHANNEL))
    ...
    # awg.set_sweep_start_hold_time(CHANNEL, 0.1)  # Expect NotImplementedError if uncommented
    ...
    # print("start hold:", awg.get_sweep_start_hold_time(CHANNEL).latest)  # Expect NotImplementedError if uncommented
    ...
    awg.set_sweep(CHANNEL, SweepType.LOG)
    print("sweep type:", awg.get_sweep_type(CHANNEL))
    time.sleep(WAIT_TIME)
    ...
    for ch in range(1, NUM_CHANNELS + 1):
        try:
            ...
            print(f"ignoring settings conflict during cleanup: {exc}")
    awg.close()
