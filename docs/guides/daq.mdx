...
`start()` on `InstroDAQ` starts counter input acquisition, and `stop()` stops it. Call `start()` before you read a counter.
...
`configure_frequency_counter_input()` reports Hz, `configure_period_counter_input()` and `configure_pulse_width_counter_input()` report seconds, and `configure_pulse_count_counter_input()` reports number of pulses.

Counter inputs are ordinary input channels, so `read()` and `read_batch()` serve them alongside analog and digital aliases.

### Counter Output

Configure the pulse shape, then start the train. `start()` and `stop()` on `InstroDAQ` do not control counter outputs. You must start and stop each output with `start_counter_output()` and `stop_counter_output()`. A finite train stops itself after `n_pulses`; a continuous one runs until `stop_counter_output()`.
