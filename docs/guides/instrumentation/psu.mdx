...
`channel` is keyword-only, so it can't be passed positionally by mistake.
...
| `apply(voltage, current_limit, enable=False, *, channel=N)` | Set the current limit, then the voltage, then the output state; returns `list[Command]`. Output stays off unless `enable=True`; on the default path the output is disabled first |
