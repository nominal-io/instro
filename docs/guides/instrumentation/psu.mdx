...
Every method on `PSUDriverBase` is declared with `@abc.abstractmethod`, so each concrete driver must explicitly implement or opt out of every method. For unsupported features, raise [`FeatureNotSupportedError`](/instrumentation/exceptions#featurenotsupportederror):
