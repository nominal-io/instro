...
These parameters apply to the direct Python constructor above, not the JSON config fields described below.
...
- **`dataset_rid`**: Optional Nominal dataset RID keyword argument. When set, automatically publishes measurements to that dataset.

### From a JSON Config File

`InstroPSU` can also be constructed from a JSON config file, which removes the need to write any Python setup code: 
...
```json
"publishers": [
  { "type": "NominalCorePublisher", "dataset_rid": "<your dataset RID>" },
  { "type": "FilePublisher", "directory": "psu_data", "format": "csv" }
]
```

An optional top-level `timing` section (`{"poll_interval": 1.0}`) sets the background daemon's polling interval. To construct from a Python dict or a raw JSON string instead:

```python
InstroPSU.from_dict(data)
InstroPSU.from_json_str(json_string)
```

JSON configs are validated strictly: `PSUConfig` forbids any field not listed above (`device`, `driver`, `timing`, `publishers`), so there is no `**kwargs`-style escape hatch from JSON. Set default tags via the direct Python constructor instead.
...
| `from_json(path)` | Construct from a JSON config file |
| `from_json_str(json_str)` | Construct from a JSON string |
| `from_dict(data)` | Construct from a Python dict |
