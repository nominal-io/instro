...
```python
commands = psu.apply(voltage=12.0, current_limit=0.5, channel=1)
```

Pass `enable=False` to configure a channel while leaving its output off. In that case the output is disabled **first**, so an already-live channel never sees the new setpoints — `apply()` reorders the steps rather than simply passing `enable` through to the end.
...
# Configure channel 1
psu.apply(voltage=12.0, current_limit=0.5, channel=1)
...
|--------|------------|------|
| `apply(...)` | `ch{N}.current.cmd`, `ch{N}.voltage.cmd`, `ch{N}.enabled.cmd` (`enabled` first when `enable=False`) | command (one per step) |
...
| `apply(voltage, current_limit, channel=N, enable=True)` | Set the current limit, then the voltage, then the output state; returns `list[Command]`. Disables the output first when `enable=False` |
