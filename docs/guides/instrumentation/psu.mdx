...
### Bringing up a channel

Step 3 is almost always the same three commands, in an order that matters: the current limit has to land before the voltage it guards. `apply()` does all three in that order and returns the three `Command` objects it published:

```python
commands = psu.apply(current_limit=0.5, voltage=12.0, enable=True, channel=1)
```

The output stays **off** unless `enable=True` is passed. On that default path the output is disabled *before* the new setpoints are written, so an already-live channel never sees them — `apply()` reorders the steps rather than passing `enable` through to a trailing `output_enable`. Energizing a channel is therefore always explicit:

```python
psu.apply(current_limit=0.5, voltage=12.0, channel=1)               # configured, output off
psu.apply(current_limit=0.5, voltage=12.0, enable=True, channel=1)  # configured and live
```

Every argument is keyword-only, so `voltage` and `current_limit` can't be swapped by position and the output can't be energized without `enable=True` spelled out at the call site.

The whole sequence runs under a single hold of the instrument lock, so a background daemon can't poll a half-applied channel.

<Warning>
`apply()` is not atomic on the instrument. If a step fails (value out of range, transport timeout), the earlier steps have already been committed and the channel is left in a state the caller can't infer from the exception. Treat any exception from `apply()` as "channel state unknown" and re-establish it explicitly. The commands for the steps that did land are still published, so the dataset records how far the sequence got.
</Warning>
...
# Configure channel 1
psu.apply(current_limit=0.5, voltage=12.0, enable=True, channel=1)
...
|--------|------------|------|
| `apply(...)` | `ch{N}.current.cmd`, `ch{N}.voltage.cmd`, `ch{N}.enabled.cmd` (`enabled` comes first unless `enable=True`) | command (one per step) |
...
| `apply(*, current_limit, voltage, enable=False, channel=N)` | Set the current limit, then the voltage, then the output state; returns `list[Command]`. Output stays off unless `enable=True`; on the default path the output is disabled first |
