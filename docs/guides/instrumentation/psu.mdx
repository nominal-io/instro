...
The output stays **off** unless you pass `enable=True`. On that default path the output is disabled *before* the new setpoints are written, so an already-live channel never sees them — `apply()` reorders the steps rather than passing `enable` through to a trailing `output_enable`. Energizing a channel is therefore always explicit:
