...
Step 3 is almost always the same three commands, in an order that matters: the current limit has to land before the voltage it guards. `apply()` does all three in that order:

```python
psu.apply(voltage=12.0, current_limit=0.5, channel=1)
```

Pass `enable=False` to configure a channel while leaving its output off. `apply()` is a thin wrapper over `set_current_limit()`, `set_voltage()`, and `output_enable()`, and publishes the same three command channels those calls would.

`channel` defaults to `1` on every `InstroPSU` method, so single-channel supplies can leave it out entirely:

```python
psu.apply(voltage=3.3, current_limit=0.5)
print(psu.get_voltage().latest)
```
...
|--------|------------|------|
| `apply(...)` | `ch{N}.current.cmd`, `ch{N}.voltage.cmd`, `ch{N}.enabled.cmd` | command (one per step) |
...
| `apply(voltage, current_limit, channel=N, enable=True)` | Set the current limit, then the voltage, then the output state |
