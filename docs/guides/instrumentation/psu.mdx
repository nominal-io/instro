...
`dataset_rid` and `output_file` and `visa_backend` are optional. To construct from a Python dict or a raw JSON string instead:
...
- **`output_file`**: Optional path to a directory for writing output data to a local file.
