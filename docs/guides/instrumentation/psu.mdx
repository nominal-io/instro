...
```json
{
  "name": "bench_psu",
  "vendor": "bk_9115",
  "connection": "USB0::0x2A8D::0x0101::MY12345::INSTR",
  "num_channels": 1,
...
`dataset_rid` and `output_directory` and `visa_backend` are optional. To construct from a Python dict or a raw JSON string instead:
...
- **`dataset_rid`**: Optional Nominal dataset RID. When set, automatically publishes measurements to that dataset.
- **`output_directory`**: Optional path to a directory for writing output data to a local file.
- **`visa_backend`**: Optional pyvisa backend specifier (e.g. `"@py"`). Defaults to `@ivi` with fallback to `@py`.
