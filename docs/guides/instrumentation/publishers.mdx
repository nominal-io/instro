...
Every [`Measurement` and `Command`](/instrumentation/library#measurements-and-commands) is now recorded by each [`Publisher`][publisher-link] in `publishers`. An `Instrument` will `close()` all publishers when it closes.
...
class Publisher(Protocol):
    ...
    def close(self) -> None:
        ...
        ...
```
* `publish`: returns [`Measurements` and `Commands`](/instrumentation/library#measurements-and-commands).
