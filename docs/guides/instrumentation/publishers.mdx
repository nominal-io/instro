...
Publishers are the mechanism by which `instro` handles data from instruments. They define what happens to measurements and commands, whether that means sending data to Nominal Core, streaming to Nominal Connect, writing to files, or implementing custom behavior.

Every instrument can have multiple publishers attached, and data flows automatically: **Instrument -> Publishers -> Destinations**.

An attached publisher is an exclusive resource owned by that instrument. Closing the instrument closes its publishers. Do not attach the same ordinary publisher instance to more than one instrument. Use [`SharedPublisher`](#sharedpublisher) when multiple instruments need to publish to one underlying destination.
...
By adding a `NominalCorePublisher` to the `publishers` list, every measurement and command is now recorded in your Nominal Core dataset. The `InstroDAQ` instance owns that publisher and closes it when `daq.close()` runs.
<Note>
**Important note about publishers**

Data is published as a direct result of an instrument method being called.

For example, when `InstroPSU.get_voltage()` runs, it queries the instrument for the voltage and causes all attached publishers to publish the measurement response automatically.
...
Treat each publisher instance as owned by one instrument. To share one file, stream, client, or other sink across instruments, wrap it in `SharedPublisher` and attach a clone to each additional instrument.
</Note>
...
The owning instrument calls `close()` during teardown. Implement `close()` to release resources owned by the publisher, such as files, clients, sockets, or background workers. A publisher should not assume it can be reused after `close()` has run.
...
Streams measurements and commands to a live terminal monitor running in a separate process. Start the monitor with `instro monitor` and attach a `MonitorPublisher` to any instrument; every channel that instrument publishes appears in the monitor's table with its latest value, age, and sample rate. Instruments constructed later in the run, or in other scripts, show up as soon as they publish.

**When to use:**
- You want to watch what a running script is publishing without writing files or streaming to Nominal
- You are bringing up a new driver or bench and want a live view of every channel
- You have several scripts running and want one view across all of them

**Constructor Parameters:**

```python
MonitorPublisher(
    host: str = "127.0.0.1",          # Address the monitor listens on
    port: int = 47251,                # Port the monitor listens on
    max_queue_size: int = 1000,       # Frames buffered before the oldest are dropped
    reconnect_interval: float = 1.0,  # Seconds between connection attempts
)
```

The publisher never blocks or raises into the instrument. A background thread sends frames as newline-delimited JSON over loopback TCP. If no monitor is listening it logs one warning, drops frames, and keeps retrying, so a script runs the same way whether or not the monitor is open. Under load the oldest frames are dropped first; the monitor shows latest values, so dropped history is not missed.
...
psu = InstroPSU(
    name="main",
    driver=BK9115("USB0::0x2EC7::0x9115::123456::INSTR"),
    num_channels=1,
...
Like every publisher, a `MonitorPublisher` belongs to one instrument. To feed several instruments through one connection, wrap it in [`SharedPublisher`](#sharedpublisher); otherwise give each instrument its own, and the monitor merges them. See [instro monitor](/instrumentation/cli#instro-monitor) for the monitor's controls.

<Note>
The monitor is a local debugging aid, not a data path. Data is held in memory in the monitor process and discarded when it exits. Use `NominalCorePublisher`, `NominalConnectPublisher`, or `FilePublisher` to record a run.
</Note>


## Attaching Publishers

Publisher attachment transfers ownership to the instrument. Use each publisher instance with one instrument unless the instance is a `SharedPublisher` handle.
...
Keep custom publisher ownership simple. The instrument that receives the publisher is responsible for closing it. Put teardown logic in `close()`, and do not reuse a custom publisher instance after its owning instrument has closed.

`data.channel_data`'s values may be `list[str]` as well as `list[float]` — a categorical read (a mode name or state reported back from the instrument) publishes as a string-valued `Measurement`. `PrintChannelPublisher` above handles this for free since it only prints `data.latest`; a custom publisher that does arithmetic on a channel's values needs to check the value's type first, the same way it would check a channel's presence.
...
Publisher wrappers modify the behavior of other publishers by adding buffering, asynchronous processing, or explicit shared ownership. They are composable: wrap any publisher to add new capabilities.
