...
Every publisher receives a [**`Data`**](https://nominal-io.github.io/instro/reference/types/#instro.lib.types.Data): channel data, timestamps, optional tags, and a `type` field. Instruments emit one of two subclasses:

- [**`Measurement`**](https://nominal-io.github.io/instro/reference/types/#instro.lib.types.Measurement): Data read from the instrument (e.g., analog input readings, voltage measurements). `type` is `DataType.MEASUREMENT`.
- [**`Command`**](https://nominal-io.github.io/instro/reference/types/#instro.lib.types.Command): A command sent to the instrument (e.g., setting voltage, enabling output). `type` is `DataType.COMMAND` and every channel holds one value.

A publisher that treats reads and writes differently checks `data.type`. The built-in publishers handle both the same way and never write `type` to the wire.
