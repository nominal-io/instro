...
- `CONSTANT`: pass the reference junction temperature in °C via `cjc_temp`.
- `CHANNEL`: not supported by this driver.


The T4's 12-bit ADC cannot resolve a bare thermocouple, so LabJack recommends using an [LJTick-InAmp](https://support.labjack.com/docs/using-a-thermocouple-with-the-t4#Overview). The driver backs the amplifier's gain and offset out of the measured volts with the `tc_input_scaler` constructor argument, which defaults to the InAmp's x51 gain and 1.25 V offset jumpers:

```python
from instro.daq.scaling.scaling import ReverseLinearScaler

driver = LabJackTSeriesDriver(
    device_id="440020473",
