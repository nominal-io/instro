...
<Note>
**Driver support.** NI-DAQmx implements every typed method above. LabJack T-Series implements the typed voltage, thermocouple, and digital methods; current input/output is not supported by LabJack hardware, so the driver raises `NotImplementedError` and points to the [measuring current app note](https://support.labjack.com/docs/measuring-current-app-note) (measure current as voltage across a shunt resistor with a voltage input and a scaler). The other bundled DAQ drivers (MCC, Keysight 34980A) do not implement the typed methods yet; with those, use `configure_analog_channel` / `configure_digital_line`.
</Note>
...
The methods below back `InstroDAQ`'s [typed channel configuration](#typed-channel-configuration). Of the packaged drivers, only NI-DAQmx and LabJack T-Series implement them (LabJack's current methods raise `NotImplementedError` because the hardware measures voltage only).
