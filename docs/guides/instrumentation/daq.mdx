...
- You call `read_analog()` or `read_digital()` when you want a sample
- No need to call `start()` or `stop()`
- Use case: Low-frequency monitoring, event-driven sampling

**Software-Timed (Background Daemon)**
- Configure the polling rate with `configure_sw_sample_rate()`
- Call `start()` to launch the background daemon, which reads every configured analog and digital input channel once per period and publishes the results
- Use case: Low-frequency continuous monitoring without a hardware sample clock

**Hardware-Timed (Buffered Acquisition)**
- Configure sample rate with `configure_hw_sample_rate()`
- Call `start()` to begin background acquisition
- Data automatically published via the background daemon, OR manually fetch with `read_analog()` / `read_digital()`
...
<Note>
**Driver support.** LabJack T-Series implements the typed voltage, thermocouple, and digital methods above. Current input/output is not supported by LabJack hardware: the driver raises `NotImplementedError` and points to the [measuring current app note](https://support.labjack.com/docs/measuring-current-app-note) (measure current as voltage across a shunt resistor with a voltage input and a scaler). The other bundled DAQ drivers (NI-DAQmx, MCC, Keysight 34980A) do not implement the typed methods yet; with those, use `configure_analog_channel` / `configure_digital_line`.
</Note>
...
For continuous hardware-timed acquisition, configure the sample rate with `configure_hw_sample_rate()`. It applies to every configured input channel. `configure_ai_sample_rate()` and `configure_ai_hw_sample_rate()` are aliases.

```python
daq.configure_hw_sample_rate(
...
For continuous acquisition without a hardware sample clock, configure a software-timed polling rate instead. The background daemon then paces `read_analog()` / `read_digital()` calls at `1 / sample_rate`. `configure_ai_sw_sample_rate()` is an alias.

```python
daq.configure_sw_sample_rate(
...
While the daemon is running, `read()` and `read_batch()` serve analog and digital channels from the daemon's channel buffer, so the same read code works whether or not the daemon is running. `read_analog()` and `read_digital()` raise instead: the daemon owns the hardware reads. Call `get_channel()` directly for more control over buffered reads (sample count, blocking for new samples, timeouts). See the [software-timed example](/instrumentation/examples/daq/daq_read_analog_sw_timed).
...
`read_digital()` mirrors `read_analog()`: it reads every configured digital input channel in one driver call, dispatching a software-timed sample or a hardware-timed buffer fetch based on the configured timing mode. `write_digital()` takes parallel lists and hands the whole set to the driver in one call:

```python
# One driver round-trip for every configured line
measurement = daq.read_digital()
print(measurement.channel_data)  # {"myDAQ.limit_switch": [1.0], "myDAQ.estop": [0.0]}

command = daq.write_digital(["enable", "reset"], [1, 0])
```

`read_digital()` returns one `Measurement` per timebase cluster; `write_digital()` returns one `Command` carrying every written alias. Both talk to the driver in `bool`: written values are cast on the way down, and the driver's returned bools become the `0.0`/`1.0` in the `Measurement` on the way up. On a write, anything greater than 0 becomes `True`, everything else `False` — a value that wasn't already `0` or `1` still writes, but raises a `UserWarning`:

```python
daq.write_digital(["enable"], [5])  # UserWarning: ... are not 0 or 1; coercing to bool
```

<Note>
These are line-only — ports are not routed through `read_digital()` or `write_digital()`, and `read_batch()` / `write_batch()` still read and write digital aliases per line or port. Use `read_digital_port()` / `write_digital_port()` for ports. No packaged driver implements the `read_digital` / `fetch_digital` / `write_digital` driver methods yet.
</Note>
...
| `read_digital()` (all DI channels in one Measurement) | `{alias}` | telemetry |
| `write_digital(channels, values)` (all channels in one Command) | `{alias}.cmd` | command |
...
| `configure_hw_sample_rate(sample_rate, samples_per_channel=None)` | Set hardware timing (`configure_ai_sample_rate` and `configure_ai_hw_sample_rate` are aliases) |
| `configure_sw_sample_rate(sample_rate)` | Set the software-timed polling rate (`configure_ai_sw_sample_rate` is an alias) |
...
| `read_digital()` | Read digital input (SW-timed) or fetch from buffer (HW-timed) |
| `write_digital(channels, values)` | Write 0/1 to digital output lines by alias in one driver call |
...
The methods below back `InstroDAQ`'s [typed channel configuration](#typed-channel-configuration). Of the packaged drivers, only LabJack T-Series implements them (its current methods raise `NotImplementedError` because the hardware measures voltage only).
...
- **`read_digital(channels)`**: Sample every DI line in `channels` in one call; return a `list[bool]` in the same order, after applying each channel's `logic`. `InstroDAQ` turns those bools into the `0.0`/`1.0` published on the wire.
- **`write_digital(channels, values)`**: Drive DO `channels[i]` to `values[i]` in one call. `values` is a `list[bool]` — `InstroDAQ` has already cast and range-warned, so the driver only applies `logic` and writes.
