...
While the background daemon is running, analog aliases are served from the daemon's channel buffer instead of triggering a hardware read. The call blocks for one fresh batch: `samples_per_channel` samples per alias when hardware timing is configured, or one sample per alias per daemon poll when software timing is configured. The timeout scales to two batch durations, with a floor of 10 seconds, so a slow acquisition does not time out. Call `get_channel()` directly when you need a different sample count, blocking behavior, or timeout.

<Warning>
**Keysight 34980A: buffered batch reads can mix adjacent scans**

The 34980A scans channels one at a time and publishes one `Measurement` per channel, so a buffered `read_batch()` of two or more analog aliases can return the previous scan for the later aliases, with timestamps one scan period apart.
Read one alias at a time with `read()` when cross-channel time alignment matters. Other DAQ drivers publish every analog channel in one `Measurement` and are not affected.
</Warning>
