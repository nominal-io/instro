...
`read()` accepts a single alias, a list of aliases, or `None` for every configured input channel. It returns a `dict` keyed by alias, with each value the channel's `Measurement`. Analog aliases are served from one `read_analog()` call and digital aliases from one `read_digital()` call; each reads every configured channel of its type, and `read()` projects out the requested aliases.
...
`read()` and `write()` route digital aliases through `read_digital()` / `write_digital()`. `read_digital()` mirrors `read_analog()`: it reads every configured digital input channel in one driver call, dispatching a software-timed sample or a hardware-timed buffer fetch based on the configured timing mode. `write_digital()` takes parallel lists and hands the whole set to the driver in one call. Both are usable directly:
...
<Note>
These are line-only — ports are not routed through `read()`, `write()`, `read_digital()`, or `write_digital()`. Use `read_digital_port()` / `write_digital_port()` for ports. No packaged driver implements the `read_digital` / `fetch_digital` / `write_digital` driver methods yet.
</Note>
...
Different vendors configure timing differently, but InstroDAQ users must always configure a timing mode before `start()`. Hardware timing (`configure_ai_hw_sample_rate()`) reaches the driver through `configure_ai_hw_timing()`; software timing (`configure_ai_sw_sample_rate()`) is handled entirely by `InstroDAQ`, which paces the daemon around the driver's `read_analog()`.
