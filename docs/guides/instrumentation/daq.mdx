...
- **`configure_ai_thermocouple_channel(channel)`**: Configure a thermocouple input channel from an `AnalogThermocoupleChannel` (thermocouple type plus cold-junction config); record on `self._ai_channels`. The driver owns unit handling: `channel.cjc_temp` arrives in `channel.unit`, and the driver must return samples in `channel.unit`. `kelvin_to_unit` / `unit_to_kelvin` in `instro.daq.scaling.thermocouple` convert to and from Kelvin for devices whose SDK is Kelvin-only; a device that accepts a temperature unit natively should program `channel.unit` instead and skip the conversion. Default raises `NotImplementedError`.
