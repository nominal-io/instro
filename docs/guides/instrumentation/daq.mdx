...
The analog methods below back `InstroDAQ`'s [typed channel configuration](#typed-channel-configuration). NI-DAQmx and LabJack T-Series implement them. The Keysight 34980A implements `configure_ai_voltage_channel` only; every other typed analog method raises `NotImplementedError`.
