...
`read()` accepts a single alias, a list of aliases, or `None` for every configured input channel. It returns a `dict` keyed by alias, with each value the channel's `Measurement`. Analog aliases are served from one batched analog read; digital aliases are read per line or port.
...
`write()` writes `values[i]` to `channels[i]`, routing each to analog or digital output by channel type. Pass a single alias and value, or parallel lists to write several channels at once. It returns the resulting `Command` for a single alias, or a list of `Command` objects for a list of aliases.
...
Digital values are coerced to `int`. The `channels` and `values` lists must be the same length; a mismatch raises `ValueError`. Relays are not routed through `write()`; use `open_relay()` and `close_relay()`.
