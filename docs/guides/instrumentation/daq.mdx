...
For continuous hardware-timed acquisition, configure the sample rate with `configure_hw_sample_rate()`. It applies to every configured input channel.
...
For continuous acquisition without a hardware sample clock, configure a software-timed polling rate instead. The background daemon then paces `read_analog()` / `read_digital()` calls at `1 / sample_rate`.
...
| `configure_hw_sample_rate(sample_rate, samples_per_channel=None)` | Set hardware timing |
| `configure_sw_sample_rate(sample_rate)` | Set the software-timed polling rate |
...
- **`read_digital()` / `fetch_digital()`**: Sample every configured DI channel in one call, lines and ports alike
