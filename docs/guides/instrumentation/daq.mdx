...
It's common for the data being read by an analog input channel to need scaling in order to represent a real world physical phenomenon. For example, a 0-5 volt sensor measuring pressure.
...
- **direction**: Use `Direction.INPUT` for digital input lines or `Direction.OUTPUT` for output lines.
...
- **port_width** *(optional)*: Set this only if you're configuring multiple digital lines as a single port (parallel I/O), not for individual line configuration.

These parameters ensure that your digital channel correctly maps to the desired input/output hardware and logical behavior. For example, to configure a digital input line with a custom logic threshold:
```python
...
| `configure_digital_channel(direction, physical_channel, alias, logic, logic_level=None)` | Configure a digital I/O channel |
...
Each of the methods below must (a) program the device and (b) record the resulting channel on the driver's own dict (`self.ai_channels`, `self.di_channels`, etc.) so later calls (`read_analog`, `start`, etc.) can find it. See [Driver-owned state](#driver-owned-state) for the full rationale.

- **`define_digital_channel(...)`**: Create a digital line or digital port channel type.
- **`configure_ai_channel(channel)`**: Configure an analog input channel on the hardware; record on `self.ai_channels[channel.alias]`.
- **`configure_ao_channel(channel)`**: Configure an analog output channel; record on `self.ao_channels[channel.alias]`. Default is a no-op for drivers that don't support AO.
- **`configure_di_channel(channel)`**: Configure a digital input channel; record on `self.di_channels[channel.alias]`.
- **`configure_do_channel(channel)`**: Configure a digital output channel; record on `self.do_channels[channel.alias]`.

### Timing Configuration

- **`configure_ai_hw_timing(hw_timing_config)`**: Program hardware-timed sampling on the device, then record on `self.ai_hw_timing_config` so `start`/`fetch_analog` can read it back.
...
**The driver is the single source of truth for every channel and every timing config that has been configured.** `InstroDAQ` does not hold its own copies — `daq.ai_channels`, `daq.ai_hw_timing_config`, and friends are `@property` proxies that delegate straight to `self._driver`. There is no back-channel into `InstroDAQ`, and the driver does not import `InstroDAQ`.

To keep every driver consistent, `DAQDriverBase.__init__` initializes the dicts and slots; concrete drivers call `super().__init__()` once and then **populate** them inside their `configure_*` methods.

#### State on every driver

`DAQDriverBase.__init__` initializes the following — populate them inside the matching `configure_*` method, and read from them anywhere the driver needs to know what's configured:

| Attribute | Type | Populated in | Read by (typical) |
|---|---|---|---|
| `ai_channels` | `dict[str, AnalogChannel]` | `configure_ai_channel` | `read_analog`, `fetch_analog`, `start`, … |
| `ao_channels` | `dict[str, AnalogChannel]` | `configure_ao_channel` | `write_analog_value` |
| `di_channels` | `dict[str, DigitalChannel]` | `configure_di_channel` | digital-input read paths |
| `do_channels` | `dict[str, DigitalChannel]` | `configure_do_channel` | digital-output write paths |
| `relay_channels` | `dict[str, RelayChannel]` | `define_relay_channel` (base default already records; overrides must too) | `open_relay` / `close_relay` |
| `ai_hw_timing_config` | `HWTimingConfig \| None` | `configure_ai_hw_timing` | `start`, `fetch_analog` |
| `ao_hw_timing_config` / `di_hw_timing_config` / `do_hw_timing_config` | `HWTimingConfig \| None` | (forward-compat; unused today) | — |
| `points_in_buffer` | `int` | `fetch_analog` (per-call) | `InstroDAQ.get_points_in_buffer` |
...
class MyDriver(DAQDriverBase):
    def __init__(self, resource: str) -> None:
        super().__init__()  # initializes ai_channels, ao_channels, di_channels,
                            # do_channels, relay_channels, *_hw_timing_config slots,
    ...
    def fetch_analog(self) -> Any:
        config = self.ai_hw_timing_config  # read driver-owned state
        for ch in self.ai_channels.values():
            ...
```

The same `self.<dict>[channel.alias] = channel` pattern applies to `configure_ao_channel`, `configure_di_channel`, and `configure_do_channel`. `DAQDriverBase.define_relay_channel`'s default builds a `RelayChannel` and records it on `self.relay_channels` already — only override if your hardware needs different parsing, and ensure your override records too.
...
def channels(self) -> list[DAQChannel]:
    """Aggregate of every configured AI/AO/DI/DO channel (excludes relays)."""
    return [
...
End users see `daq.ai_channels` as a dict, but mutating it would be mutating the driver's dict — treat these as read-only views. Users who want the underlying driver can reach it via `daq.driver`.
...
- **NI-DAQmx**: Sample rate is configured on the device prior to starting an acquisition. The driver implements this in `configure_ai_hw_timing()` by calling DAQmx's `task.timing.cfg_samp_clk_timing()` and then stores the config on `self.ai_hw_timing_config`.

- **LabJack LJM**: Sample rate is configured when `ljm.eStreamStart()` is called. The driver records the `HWTimingConfig` in `configure_ai_hw_timing()` and reads it back from `self.ai_hw_timing_config` inside `start()`.
