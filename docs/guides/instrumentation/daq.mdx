...
Counter driver support:

| Driver | Finite pulse train | Continuous pulse train | Edge counting | Frequency | Period | Pulse width |
|--------|:---:|:---:|:---:|:---:|:---:|:---:|
| NI-DAQmx | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| LabJack T-Series | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| MCC | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
...
No driver implements counters yet. `InstroDAQ` defines the interface, and every `DAQDriverBase` counter method raises `NotImplementedError` until a vendor driver overrides it.
...
daq.configure_pulse_count_counter_input(
    physical_channel="/Dev1/PFI8",
    alias="revolutions",
    edge_type=Edge.RISING,
    count_up=True,
    counter_source="Dev1/ctr0",
)
...
`physical_channel` names the terminal the signal arrives on or leaves by. `counter_source` names the counter that does the work. NI requires it, because a counter routes to a PFI terminal.
...
| `configure_pulse_count_counter_input(physical_channel, ...)` | Configure a counter input that counts edges |
| `configure_frequency_counter_input(physical_channel, ...)` | Configure a counter input that measures frequency (Hz) |
| `configure_period_counter_input(physical_channel, ...)` | Configure a counter input that measures period (seconds) |
| `configure_pulse_width_counter_input(physical_channel, ...)` | Configure a counter input that measures pulse width (seconds) |
| `configure_finite_counter_output(physical_channel, pulse_config, n_pulses, ...)` | Configure a counter output that emits `n_pulses` and stops |
| `configure_continuous_counter_output(physical_channel, pulse_config, ...)` | Configure a counter output that pulses until stopped |
| `read_counter(channel)` | Read a counter input's configured measurement |
| `start_counter_output(channel)` | Start the pulse train on a counter output |
| `stop_counter_output(channel)` | Stop the pulse train and release the counter |
| `wait_for_counter_output(channel, timeout=None)` | Block until a finite pulse train completes |
