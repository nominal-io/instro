...
2. **Channel configuration**: Translate generic channel configurations into vendor-specific setup
3. **Timing abstraction**: Implement both software-timed reads and hardware-timed buffered acquisition. `InstroDAQ` will manage the background thread for the hardware-timed acquisition.
4. **Data format conversion**: Convert vendor-specific data formats to `Measurement` objects
5. **Constraint validation**: Handle vendor-specific hardware constraints and raise clear exceptions when the vendor library cannot.
...
- **`configure_ai_channel(channel)`**: Configure an analog input channel on the hardware.
- **`configure_di_channel(channel)`**: Configure a digital input channel on the hardware.
- **`configure_do_channel(channel)`**: Configure a digital output channel on the hardware.

### Timing Configuration

- **`configure_ai_hw_timing(hw_timing_config)`**: Configure hardware-timed sampling.
...
### InstroDAQ State Information

Drivers have access to the state information `InstroDAQ` via the `APIInstroDAQ` interface available as an attribute, `self.hal`. `InstroDAQ` keeps track of the settings a user has configured so that the driver doesn't have to keep track of state except where necessary due to vendor implementation details.
...
```
This is useful for differences in how vendors expect someone to interact with their device.

#### Example: Vendor Difference Solved with APIInstroDAQ
...
- **NI-DAQmx**: Sample rate is configured on the device prior to starting an acquisition. The driver shall implement this in `configure_ai_hw_timing()` by calling DAQmx's `task.timing.cfg_samp_clk_timing()`.

- **LabJack LJM**: Sample rate is configured when `ljm.eStreamStart()` is called. Since `InstroDAQ` stores the configuration information when `InstroDAQ.configure_ai_sample_rate()` is called, the `start()` implementation can grab the `HWTimingConfig` from `InstroDAQ` and configure the device.
