...
| `configure_thermocouple_input(physical_channel, tc_type, *, alias, range_min, range_max, scaler, cjc_source, cjc_temp, cjc_channel, unit, tc_input_scaler)` | `AnalogThermocoupleChannel` | `configure_ai_thermocouple_channel` |
...
daq.configure_thermocouple_input("AIN0", TC_TYPE.K, alias="tc0")
```
...
Readings return in `unit` (Celsius by default). A channel `scaler` applies to the converted temperatures.
