...
<Note>
**Driver support.** NI-DAQmx implements every typed method above. LabJack T-Series implements the typed voltage, thermocouple, and digital methods; current input/output is not supported by LabJack hardware, so the driver points to the [measuring current app note](https://support.labjack.com/docs/measuring-current-app-note).
</Note>
