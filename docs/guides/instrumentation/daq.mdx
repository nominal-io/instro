...
`InstroDAQ` supports adding a `Scaler` object when you configure your analog channel. The `Measurement` objects published and returned by `read_analog` will contain these scaled values.
...
The `read_analog()` method always returns a list with one `Measurement` object per timebase cluster. Each object contains:
...
- **`read_analog() -> list[Measurement]`**: Perform software-timed analog read
