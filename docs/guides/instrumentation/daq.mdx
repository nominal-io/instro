...
### Batched Digital Input

`read_digital()` mirrors `read_analog()`: it reads every configured digital input channel in one driver call, dispatching a software-timed sample or a hardware-timed buffer fetch based on the configured timing mode. Lines and ports are both covered — a line channel carries `0`/`1`, a port channel carries its whole N-bit integer:

```python
# One driver round-trip for every configured digital input
measurement = daq.read_digital()
print(measurement.channel_data)  # {"myDAQ.limit_switch": [1.0], "myDAQ.status_bits": [15.0]}
```

`read_digital()` returns one `Measurement` per timebase cluster. There is no batched digital write; use `write_digital_line()` and `write_digital_port()`.

<Note>
No packaged driver implements the `read_digital` / `fetch_digital` driver methods yet, and `read_batch()` still reads digital aliases per line or port.
</Note>
...
| `read_digital()` | Read every digital input channel, line and port (SW-timed) or fetch from buffer (HW-timed) |
...
- **`read_digital()` / `fetch_digital()`**: Sample (or fetch buffered samples for) every configured DI channel in one call, lines and ports alike, after applying each channel's `logic`. Return a vendor payload for `_read_to_measurements` to unpack, with each line carrying `0`/`1` and each port its whole N-bit integer.
