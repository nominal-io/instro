...
<Note>
**Driver support.** LabJack T-Series implements the typed voltage, thermocouple, and digital methods above. Current input/output is not supported by LabJack hardware: the driver raises `NotImplementedError` and points to the [measuring current app note](https://support.labjack.com/docs/measuring-current-app-note) (measure current as voltage across a shunt resistor with a voltage input and a scaler). MCC implements the typed voltage, thermocouple, current-input, and digital methods; current output is not supported. The other bundled DAQ drivers (NI-DAQmx, Keysight 34980A) do not implement the typed methods yet; with those, use `configure_analog_channel` / `configure_digital_line`.
</Note>
