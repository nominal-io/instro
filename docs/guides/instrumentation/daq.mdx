...
<Note>
**Driver support.** NI-DAQmx implements every typed method above. The other bundled DAQ drivers (LabJack T-Series, MCC, Keysight 34980A) do not yet — with those, use `configure_analog_channel` / `configure_digital_line`.
</Note>
...
The methods below back `InstroDAQ`'s [typed channel configuration](#typed-channel-configuration). Of the packaged drivers, only NI-DAQmx implements them.
