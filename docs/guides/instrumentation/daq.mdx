...
Hardware and software timing are mutually exclusive on one instance: configuring the second mode raises `HWTimingException`. To run both against one device, build a separate `InstroDAQ` per mode with non-overlapping channels: see the [NI hardware- and software-timed example](/instrumentation/examples/daq/daq_hw_and_sw_timed_ni).
...
For continuous hardware-timed acquisition, configure the sample rate. `configure_ai_sample_rate()` is an alias for `configure_hw_sample_rate()`.
...
While the daemon is running, `read_analog()` raises: the daemon owns the reads. Use `get_channel()` to pull buffered samples instead. See the [software-timed example](/instrumentation/examples/daq/daq_read_analog_sw_timed).
...
`read()` accepts a single alias, a list of aliases, or `None` for every configured input channel. It returns a `dict` keyed by alias, with each value the channel's `Measurement`. Analog aliases are served from one batched `read_analog()`; digital aliases from one batched `read_digital()`.
...
# A single channel
result = daq.read("v0")
print(result["v0"].latest)

# Several channels of mixed type
result = daq.read(["v0", "limit_switch"])
...
`write()` writes `values[i]` to `channels[i]`, routing each to `write_analog_value()` or a single batched `write_digital()` by channel type. Pass a single alias and value, or parallel lists to write several channels at once. It returns the resulting `Command` for a single alias, or a list of `Command` objects for a list of aliases.
...
daq.write(["ao0", "enable"], [2.2, 1])
```

Every alias is validated before anything is written, so a bad alias can't leave earlier channels already driven. The `channels` and `values` lists must be the same length; a mismatch raises `ValueError`. Relays are not routed through `write()`; use `open_relay()` and `close_relay()`.

### Batched Digital I/O

`read()` and `write()` route digital aliases through `read_digital()` / `write_digital()`, which take parallel lists and hand the whole set to the driver in one call. Both are usable directly:

```python
# One driver round-trip for every line
measurement = daq.read_digital(["limit_switch", "estop"])
print(measurement.channel_data)  # {"myDAQ.limit_switch": [1.0], "myDAQ.estop": [0.0]}

command = daq.write_digital(["enable", "reset"], [1, 0])
```

`read_digital()` returns one `Measurement` carrying every requested alias; `write_digital()` returns one `Command` carrying every written alias. Both talk to the driver in `bool`: written values are cast on the way down, and the driver's returned bools become the `0.0`/`1.0` in the `Measurement` on the way up. On a write, anything greater than 0 becomes `True`, everything else `False` — a value that wasn't already `0` or `1` still writes, but raises a `UserWarning`:
...
<Note>
These are line-only — ports are not routed through `read()`, `write()`, `read_digital()`, or `write_digital()`. Use `read_digital_port()` / `write_digital_port()` for ports. No packaged driver implements the `read_digital` / `write_digital` driver methods yet.
</Note>
...
| `read(channels)` (per channel) | `{alias}` | telemetry |
| `write(channels, values)` (per channel) | `{alias}.cmd` | command |
| `read_digital(channels)` (all channels in one Measurement) | `{alias}` | telemetry |
...
| `configure_hw_sample_rate(sample_rate, samples_per_channel=None)` | Set hardware timing for analog input (`configure_ai_sample_rate` is an alias) |
| `configure_sw_sample_rate(sample_rate)` | Set the software-timed polling rate for analog input |
...
| `read(channels=None)` | Read input channel(s) by alias; returns `{alias: Measurement}` |
| `write(channels, values)` | Write value(s) to output channel(s) by alias, paired by index |
| `read_digital(channels)` | Read digital input lines by alias in one driver call |
...
- **`configure_hw_timing(hw_timing_config)`**: Program hardware-timed sampling on the device, then record on `self._ai_hw_timing_config` so `start`/`fetch_analog` can read it back.
...
| `_ai_hw_timing_config` | `ai_hw_timing_config` | `HWTimingConfig \| None` | `configure_hw_timing` | `start`, `fetch_analog` |
...
class MyDriver(DAQDriverBase):
    ...
    def configure_hw_timing(self, hw_timing_config: HWTimingConfig) -> None:
        ...  # program the device
...
Different vendors configure timing differently, but InstroDAQ users must always configure a timing mode before `start()`. Hardware timing (`configure_hw_sample_rate()`) reaches the driver through `configure_hw_timing()`; software timing (`configure_sw_sample_rate()`) is handled entirely by `InstroDAQ`, which paces the daemon around the driver's `read_analog()`.

- **NI-DAQmx**: Sample rate is configured on the device prior to starting an acquisition. The driver implements this in `configure_hw_timing()` by calling DAQmx's `task.timing.cfg_samp_clk_timing()` and then stores the config on `self._ai_hw_timing_config`.

- **LabJack LJM**: Sample rate is configured when `ljm.eStreamStart()` is called. The driver records the `HWTimingConfig` in `configure_hw_timing()` and reads it back from `self._ai_hw_timing_config` inside `start()`.
