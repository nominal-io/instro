...
Values may be `float`, `int`, or `bool`, and every value is validated against its target channel before anything is written: analog outputs require a finite number, digital lines require `0` or `1` (in any of the three forms), and digital ports require an integer. An invalid value — or a `channels`/`values` length mismatch — raises `ValueError` with no channels written. Digital values are coerced to `int`. Relays are not routed through `write()`; use `open_relay()` and `close_relay()`.
...
| `write_batch(channels, values)` | Write values to output channels by alias, paired by index; returns `list[Command]` |
