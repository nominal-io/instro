...
Use a protocol client when the device interface is already defined by a protocol and a config file can name the addresses, tags, scaling, and polling interval. The config becomes the portable definition of how Nominal reads the device; teams can version it with the test setup and change polling without editing Python.
...
- **[EtherNet/IP](/instrumentation/protocols/ethernetip)**: Unstable Allen-Bradley PLC tag access with config-driven scalar reads, writes, and poll-enabled background collection.
