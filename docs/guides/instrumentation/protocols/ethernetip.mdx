...
String support is for Allen-Bradley Logix `STRING` tags encoded as `LEN` (`DINT`) plus `DATA` (`SINT[]`). String measurements are not supported by Nominal Core or Nominal Connect. Manual string tag reads and writes are supported by the native API, but not through the background polling thread.

<Note>
Current CompactLogix 5332E 1769-L32E hardware testing covers boolean and numeric scalar tags. Validate string tags against the target PLC before using them in production configs.
</Note>
