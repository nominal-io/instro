...
<Info>
The EtherNet/IP client lives under `instro.ethernetip` and is provided by the `instro-ethernetip` package (a native backend). Install it with the `instro[ethernetip]` extra; it is also included in `instro[all]`.
</Info>

## Install

Install with the EtherNet/IP extra:

```bash
uv add "instro[ethernetip]"
```

or with pip:

```bash
pip install "instro[ethernetip]"
```

The standalone package also installs directly: `pip install instro-ethernetip`.

<Note>
EtherNet/IP graduated out of the experimental `instro-unstable` package. If you used a pre-release
...
`instro[ethernetip]` instead of `instro-unstable[ethernetip]`.
</Note>
...
| Error | Meaning |
| --- | --- |
| `RuntimeError: EtherNet/IP support requires the native package 'instro-ethernetip'` | The native PyO3 package is not installed. Install `instro[ethernetip]`. |
...
- EtherNet/IP support is installed through `instro[ethernetip]` (or the standalone `instro-ethernetip` package).
