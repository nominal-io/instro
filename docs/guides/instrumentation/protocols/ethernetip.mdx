...
The config-driven API supports scalar PLC tag types. The `data_type` field is required for every tag so reads can validate that the PLC returned the expected kind before publishing. If the PLC returns a different kind than the configured `data_type`, a manual `EtherNetIP.read_tag()` raises `TypeError`; background polling logs a decode warning for that tag and continues publishing other successful reads from the same batch.
...
The public config-driven `EtherNetIP.write_tag()` method can write string command tags when `poll` is disabled:
...
If the PLC tag's actual type differs from the configured `data_type`, a manual `EtherNetIP.read_tag()` call raises `TypeError`. During background polling, that tag is skipped for the current cycle, a decode warning is logged, and other successfully decoded tags in the same batch are still published.
