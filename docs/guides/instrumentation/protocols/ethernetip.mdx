...
PLC string tags are not part of the Python EtherNet/IP API. Do not declare `data_type: "string"` in `EtherNetIPDevice` configs, and do not pass Python `str` values to the native `EtherNetIpSession.write_tag()` API.
