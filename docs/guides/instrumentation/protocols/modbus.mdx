...
modbus_device = InstroRegisterInstrument(driver=ModbusRegisterDriver(config))
...
Most devices can be brought online with just a connection, a few registers, and `open()` + `start()`. The features below each solve a specific problem you'll run into as soon as you're working with real hardware. Use this table to jump to the right section.

| If you're trying to... | Reach for... |
| --- | --- |
| Stream live data to Nominal Core with the shortest possible setup | [`open()` + `start()`](#lifecycle-pattern) + [`add_publisher()`](#creating-an-instroregisterinstrument-instance) |
...
| Share one device config across test benches with different networking | [Merge `connection` into `ModbusConfig`](#creating-an-instroregisterinstrument-instance) at construction |
...
2. **Instantiate**: Construct `ModbusRegisterDriver(config)` and wrap it in `InstroRegisterInstrument(driver=...)`
3. **`open()`**: Establish connection to the Modbus device
4. **`start()`**: Begin background polling (requires a `timing` section in the config)
...
Modbus devices can vary in how they order bytes within multi-register values. The driver provides three swap options to handle these differences:
...
Modbus is configured through a JSON file (or a `ModbusConfig` built in Python). Every config file must include:
...
Modbus TCP connections drop for all the usual reasons, such as network blips, device reboots, and power cycles on a PLC. The driver handles this transparently. When a read or write hits a transport error, the driver closes the dead socket and pymodbus opens a fresh one on the next operation. No custom retry loop, no explicit `open()` after a disconnect.

Your code just needs to tolerate the occasional `OSError` or `ConnectionError` being raised from `read()` / `write()`. When background polling is running via `start()`, transient errors from a single poll are logged without tearing down the worker. The next tick will succeed as soon as the device comes back.

## Prototyping Without a Device

Not every development environment has a PLC sitting on the bench. For config authoring, CI tests, and demos, the library ships with a lightweight TCP simulator you can run alongside your code:

```bash
python -m instro.utils.protocol.sim_server
```

The sim server listens on `127.0.0.1:5020` and exposes holding registers, input registers, coils, and discrete inputs seeded with representative values. Point any TCP config at it by merging the connection at construction:
...
raw = json.loads(Path("my_device.json").read_text())
config = ModbusConfig.model_validate({**raw, "connection": {"transport": "tcp", "host": "127.0.0.1", "port": 5020}})
...
# Merge the environment-specific connection into the shared config at runtime
raw = json.loads(Path("my_device.json").read_text())
...
config = ModbusConfig(
    ...
    connection=TCPConnectionConfig(host="192.168.1.100", port=502, unit_id=1),
...
device = InstroRegisterInstrument(driver=ModbusRegisterDriver(config))
device.open()
device.start()  # begin background polling (requires timing section in config)
```
...
|-----------|----------|---------|-------------|
| `driver` | Yes | | A `ModbusRegisterDriver` instance wrapping a `ModbusConfig` |
| `publishers` | No | `None` | List of publishers to attach at construction time |

<Tip>
Separate environment-specific connection details from the shared device config by building a `ModbusConfig` via `model_validate({**raw_json, "connection": connection})`. One JSON config can be shared across test benches while each environment supplies its own connection.
</Tip>
...
When a `timing` section is present in the config, call `open()` and `start()` to begin background polling:
...
raw = json.loads(Path("watlow_f4t.json").read_text())
connection = {"transport": "tcp", "host": "192.168.5.222", "port": 502}
config = ModbusConfig.model_validate({**raw, "connection": connection})
f4t = InstroRegisterInstrument(driver=ModbusRegisterDriver(config))
f4t.open()
f4t.start()
...
| Method | Purpose |
|--------|---------|
| `InstroRegisterInstrument(driver, publishers=None)` | Construct the instrument from a `ModbusRegisterDriver`. |
| `ModbusRegisterDriver(config, thread_safe=False)` | Construct the driver from a `ModbusConfig`. Pass `thread_safe=True` when `start()` will run concurrently with manual reads/writes. |
...
| `read(register_id, **kwargs)` | Read a register by alias, apply scaling, return `Measurement` |
| `write(register_id, value, **kwargs)` | Write a value to a register by alias (accepts strings for `write_value_map` registers), apply reverse scaling, return `Command` |
| `read_group(group_id, **kwargs)` | Read all registers in a group in a single Modbus transaction, return `Measurement` |
| `get_channel(channel_name, length=1, wait_for_latest=False, timeout=10.0)` | Retrieve buffered poll data for a channel |
| `start()` | Start background polling daemon (requires `timing` in config) |
| `stop()` | Stop background polling daemon |

## Error Handling

The driver raises descriptive errors for common issues:
