...
daq.configure_voltage_input(physical_channel="AIN0", alias="ch_0", range_min=-10, range_max=10)
daq.configure_voltage_input(physical_channel="AIN1", alias="ch_1", range_min=-10, range_max=10)
daq.configure_ai_hw_sample_rate(sample_rate=5000, samples_per_channel=500)
...
- `InstroAWG`: arbitrary waveform generators
...
`BasicBufferedPublisher`, `QueuedPublisher`, and `SharedPublisher` wrap another publisher to add batching, background delivery, or explicit shared ownership. See [Publishers](/instrumentation/publishers) for configuration and examples.
