...
`instro` ships a command-line interface for discovering instruments on your bench and watching what running scripts publish.
...
Opens a live terminal view of every channel that running scripts publish through a [`MonitorPublisher`](/instrumentation/publishers#monitorpublisher). The monitor is its own process: start it once, then run scripts that attach the publisher. Scripts can start before or after the monitor and can come and go while it runs.

```bash
uv run instro monitor
```

Each row is one channel, grouped by the instrument name prefix. Columns show whether the last value was a measurement (`M`) or a command (`C`), the latest value, how long ago it arrived, the sample rate over the last five seconds, the total sample count, and the script connection it came from. Rows from a script that has disconnected are dimmed and marked `(gone)` so the last known value stays visible.

Press `c` to clear the table and `q` to quit.

The monitor listens on `127.0.0.1:47251` by default. Change either with `--host` and `--port`, and pass the same values to `MonitorPublisher` in the script:

```bash
uv run instro monitor --port 5000
```

The monitor never contacts instruments. It only receives what scripts send, so it cannot slow down or interfere with a running test.
