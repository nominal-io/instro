...
Each selects the function only when it isn't already active, so repeated calls in the same function cost a single measurement and publish `set_measurement_function.cmd` once. They are thin wrappers over `set_measurement_function()` + `read()` and publish exactly the same channels.
...
# Trigger a DC voltage measurement
measurement = dmm.read_dc_voltage()
print(f"DC Voltage: {measurement.latest}")

# Or select the function explicitly, to tune resolution, aperture, or range first
...
| `read_dc_voltage()` and siblings | `set_measurement_function.cmd` when the function changes, then `{function}` | command + telemetry |
...
| `read_dc_voltage()`, `read_ac_voltage()`, `read_dc_current()`, `read_ac_current()`, `read_resistance()`, `read_four_wire_resistance()` | Select the function if it isn't already active, then read |
