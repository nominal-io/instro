...
- **`curr_limit`**: current limit applied with the level. Only valid when `mode` is `CV`, mirroring the `set_level` signature. On the `BK85XXB` driver this programs the over-current protection (`CURR:PROT`), which is a latching trip — current exceeding the limit for the protection delay turns the input off — rather than a continuous CV→CC crossover limit, which the 8500B series does not expose over SCPI.
...
class BK85XXB(ELoadDriverBase):
    ...
    def set_level(self, mode: LoadMode, value: float, channel: int, curr_limit: float | None) -> None:
        # The 8500B's only SCPI current ceiling is the over-current protection trip (input
        # off once exceeded); it offers no continuous CV->CC crossover limit over SCPI.
        if curr_limit is not None:
            self._visa.write(f"CURRent:PROTection {curr_limit}")
