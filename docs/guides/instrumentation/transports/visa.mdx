...
| **`close()`** | Closes the resource and the resource manager. Idempotent. Declines and logs a warning instead if another driver still holds the connection; see [Shared ownership](#shared-ownership). |

`open()` and `close()` are both safe to call more than once. A best-effort teardown also runs on garbage collection, bypassing the shared-ownership guard, but you should not rely on this. Close explicitly in a `try`/`finally`, or wrap the driver in the `open()`/`close()` of a higher-level instrument driver.

### Shared Ownership

Some instruments expose more than one logical surface over a single connection. The EA PSB series, for example, both sources and sinks current on the same box, so it needs a PSU-shaped driver and an ELoad-shaped driver sharing one `VisaDriver`. `VisaDriver` (and `ModbusDriver`) support this through `OwnershipContext`, the base both transports inherit.

A driver that wants to share a connection calls `acquire()`/`release()` with itself as the holder, instead of `open()`/`close()` directly:
    ...
    def open(self) -> None:
        if self._visa.acquire(self):              # True only for the first owner
            try:
                ...
                # A stranded holder would make the retry's acquire report not-first-owner,
...
`acquire()` opens the connection if needed and reports whether this call made the driver the **first** owner, so one-time device setup (the box's remote lock, here) runs exactly once no matter how many surfaces share the connection. `release()` mirrors this: the connection stays open as long as any owner remains, and only the release that empties the owner list runs `on_last_release` and tears the connection down. If that one-time setup raises, the driver must `release()` itself before propagating, as above: a holder left registered by a failed `open()` makes a later `acquire()` report not-first-owner and skip the setup it still needs to run.

Construct one `VisaDriver` and hand it to both drivers to wire up the combined instrument:
...
sink.close()     # last release: SYST:LOCK OFF, then the socket closes
```

A driver that never calls `acquire()`/`release()` is unaffected. It is the sole owner by construction, so `open()`/`close()` behave exactly as in [Lifecycle](#lifecycle).
...
| `acquire(holder)` | Admit `holder` and open the connection if needed. Returns `True` if `holder` is the first owner. See [Shared ownership](#shared-ownership). |
| `close()` | Close the resource and resource manager. Idempotent. Declines and logs a warning if another driver still holds the connection. |
| `release(holder, on_last_release=None)` | Remove `holder`. The release that empties the owner list runs `on_last_release`, then tears the connection down. See [Shared ownership](#shared-ownership). |
