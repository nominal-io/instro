...
The defaults are `read="\n"` and `write="\r\n"`, which works for most SCPI instruments. Override them through [`TerminatorConfig`](#terminatorconfig) when an instrument's programming manual specifies otherwise — for example USB-TMC devices that use `"\n"` for both directions, or older instruments that expect bare `"\r"`.
...
| **Raw bytes** | `write_raw`, `read_raw`, `query_raw` | Bytes pass through unchanged on write; reads return `bytes` and are not terminator-stripped | Binary payloads — waveform downloads from oscilloscopes, image transfers, vendor-specific binary blobs |
