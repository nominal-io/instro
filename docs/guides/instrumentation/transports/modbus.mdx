...
`ModbusDriver` shares the same `acquire()`/`release()` contract as `VisaDriver`, inherited from `OwnershipContext`: multiple drivers can hold one connection, and it closes only when the last owner releases it. See [Shared ownership](/instrumentation/transports/visa#shared-ownership) for the full explanation and a worked example.
