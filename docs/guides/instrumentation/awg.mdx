...
| `set_gate_polarity(channel=N)` | `ch{N}.gate_polarity.cmd` | command |
| `set_ncycles(channel=N)` | `ch{N}.ncycles.cmd` | command |
...
| `get_burst_ncycles(channel=N)` | `ch{N}.ncycles` | telemetry |
| `get_burst_period(channel=N)` | `ch{N}.burst_period` | telemetry |

<Note>
`get_waveform()`, `get_amplitude()`, `convert_amplitude()`, `get_modulation_type()`, `get_burst_type()`, and `get_burst_polarity()` return a plain Python value (`Waveform`, `tuple[float, AmplitudeMeasurementUnit]`, `float`, `ModulationType`, `BurstType`, `GatePolarity`) directly rather than a `Measurement`, and don't publish. Every other readback listed above publishes a `Measurement` on the descriptor shown.
</Note>
...
| `set_gate_polarity(channel, gate_polarity)` | Set the gate polarity (NORM/INV) for GATED bursts on channel |
| `get_burst_polarity(channel)` | Read back the gate polarity for GATED bursts on channel |
| `set_ncycles(channel, n_cycles)` | Set the number of cycles per trigger for NCYCLE bursts on channel |
...
- **`burst_enable(channel, enable)`** / **`get_burst_state(channel)`**: Enable, disable, or read back whether burst mode is active on channel.
- **`set_burst_trigger(channel, source)`**: Set the burst trigger source (INTERNAL/EXTERNAL/MANUAL) on channel.
- **`set_burst_delay(channel, delay_s)`** / **`get_burst_delay(channel)`**: Set or read back the burst trigger delay (seconds) on channel.
- **`set_gate_polarity(channel, gate_polarity)`** / **`get_burst_polarity(channel)`**: Set or read back the gate polarity (NORM/INV) for GATED bursts on channel.
- **`set_ncycles(channel, n_cycles)`** / **`get_burst_ncycles(channel)`**: Set or read back the number of cycles per trigger for NCYCLE bursts on channel.
- **`set_burst_period(channel, period)`** / **`get_burst_period(channel)`**: Set or read back the internal burst period (seconds) on channel.
