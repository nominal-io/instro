...
| `get_modulation_state(channel=N)` | `ch{N}.modulation_state` | telemetry |
...
| `get_modulation_state(channel)` | Read back whether modulation is enabled on channel, queried directly from the instrument (active type ships as a `mod_type` tag) |
