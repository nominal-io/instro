...
- **`set_burst_trigger(channel, source)`**: Set the burst trigger source (`INTERNAL`/`EXTERNAL`/`MANUAL`).
- **`set_burst_delay(channel, delay_s)`** / **`get_burst_delay(channel)`**: Set or read back the burst trigger delay in seconds.
- **`set_gate_polarity(channel, gate_polarity)`** / **`get_gate_polarity(channel)`**: Set or read back the gate polarity (`NORM`/`INV`) for GATED bursts.
- **`set_ncycles(channel, n_cycles)`** / **`get_burst_ncycles(channel)`**: Set or read back the number of cycles per trigger for NCYCLE bursts.
