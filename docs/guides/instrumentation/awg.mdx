...
Each entry is applied on the first call to `open()`, through `set_waveform`, `set_amplitude`, and `set_offset`. Building an `InstroAWG` does not `start()` the instrument unless you pass `autostart=True`.
