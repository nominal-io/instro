...
Each entry is applied on the first call to `open()`, through `set_waveform`, `set_amplitude`, and `set_offset`. Building an `InstroAWG` does not touch the instrument unless you pass `autostart=True`.
...
A CSV path is used as written. A relative path resolves against the working directory of the process, not the config file, so use an absolute path unless you control where the process starts. The file is read when the config is parsed, so later edits to it do not affect a config you already built.
