...
Call `enable_modulation(channel)` to turn on modulation with the last-configured `set_modulation()` settings, and `disable_modulation(channel)` to turn it back off. The two are independent of `set_modulation()`: once configured, a channel can be toggled on and off any number of times without reconfiguring, and `disable_modulation` keeps the configured settings rather than clearing them.
...
| `enable_modulation(channel=N)` | `ch{N}.modulation.cmd` | command |
| `disable_modulation(channel=N)` | `ch{N}.modulation.cmd` | command |
...
| `enable_modulation(channel)` | Enable modulation on channel using its last-configured `set_modulation()` settings |
| `disable_modulation(channel)` | Disable modulation on channel, if any is active |
...
Six methods are optional and raise `NotImplementedError` by default, for drivers whose instrument doesn't support them:
