...
| `get_gate_polarity(channel)` | Read back the gate polarity for GATED bursts on channel |
...
- **`set_gate_polarity(channel, gate_polarity)`** / **`get_gate_polarity(channel)`**: Set or read back the gate polarity (NORM/INV) for GATED bursts on channel.
