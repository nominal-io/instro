...
- **Keysight**: 33521B (33500B series) via SCPI/VISA (`Keysight33500B`)
...
Default to one `_check_errors()` call per public method, after all of that call's writes/queries — cheap, and still catches anything the call did wrong. Only reach for checking after every individual write when checking late is itself a problem: `RigolDG1022Z`'s arbitrary-waveform point-by-point upload (up to 16384 individual writes) checks every point, because the error queue isn't drained and a bad point could otherwise go unnoticed for thousands of writes on a path with a known history of USB/firmware trouble on that hardware. `Keysight33500B`'s arbitrary-waveform download is a single bulk write instead, so it doesn't have that failure mode and sticks with the one-check-per-call default. That's a judgment call each driver author makes for their instrument, not a fixed rule.
