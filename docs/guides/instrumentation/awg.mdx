...
Call `disable_modulation(channel)` to turn modulation back off; it's a no-op if the channel has no active modulation.
...
| `modulate(channel=N)` | `ch{N}.modulation.cmd` | command |
...
| `modulate(channel, mod_type, shape, magnitude)` | Modulate a channel's waveform (AM/FM/PM/ASK/FSK) |
...
def modulate(self, channel: int, mod_type: ModulationType, shape: Waveform, magnitude: float) -> None:
    """Modulate channel's carrier with modulator shape; magnitude's unit and shape's effect vary by mod_type (see AWG guide)."""

def disable_modulation(self, channel: int) -> None:
    """Disable modulation on channel, if any is active."""
```
