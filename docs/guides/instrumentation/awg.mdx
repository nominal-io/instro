...
| `force_burst_trigger(channel=N)` | `ch{N}.burst_trigger_forced.cmd` | command |
| `set_burst_delay(channel=N)` | `ch{N}.burst_delay.cmd` | command |
| `set_burst_gate_polarity(channel=N)` | `ch{N}.gate_polarity.cmd` | command |
| `set_burst_ncycles(channel=N)` | `ch{N}.ncycles.cmd` | command |
...
| `force_burst_trigger(channel)` | Force the burst trigger source to `MANUAL` and fire a trigger on channel now |
...
- **`force_burst_trigger(channel)`**: Force the trigger source to `MANUAL` and fire a burst trigger on channel immediately.
