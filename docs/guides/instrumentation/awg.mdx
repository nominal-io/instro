...
`InstroAWG` can also be constructed directly from a JSON config file, which removes the need to write any Python setup code:
...
`channels` is required and must have at least one entry, keyed by 1-indexed channel number. Each entry is applied through the same public setters shown in [Basic Usage](#basic-usage) — `set_waveform`, `set_amplitude`, `set_offset`, `set_modulation`/`modulation_enable`, `set_burst*`/`burst_enable`, `set_sweep*`/`sweep_enable` — once, the first time `open()` is called:

- **`waveform`** (required): one of `sine`, `square`, `sawtooth`, `triangle`, `pulse`, `arbitrary`, or `static_value`, discriminated by a `shape` field. `arbitrary`'s `samples` accepts either an inline array of floats normalized to `[-1.0, 1.0]`, or a string path to a CSV file containing them; `sample_rate_sas` is the sample rate in samples per second.
- **`amplitude`** (optional): `{"value": <float>, "unit": "VPP" | "VP" | "VRMS" | "DBM"}`; `unit` defaults to `VPP`.
- **`offset`** (optional): DC offset in volts.
- **`modulation`** (optional): `{"type": {"name": <ModulationType>, "magnitude": <float>}, "baseband_shape": <waveform>, "enable": <bool>}`. `magnitude`'s meaning depends on `name` (depth for AM, frequency deviation for FM, etc. — see `set_modulation`).
- **`burst`** (optional): `{"type": <BurstType>, "enable": <bool>, "trigger_source": ..., "delay": ..., "gate_polarity": ..., "ncycles": ..., "period": ...}`. Only `type` and `enable` are required; the rest are mode-specific and skipped when omitted (e.g. `gate_polarity` only applies to `GATED` bursts).
...
`output_enable` cannot be set from config — enabling modulation/burst/sweep never turns on the physical output by itself, so a freshly-configured channel stays silent until you explicitly call `output_enable(channel, True)`.
...
JSON configs are validated strictly: `AWGConfig` and its nested blocks forbid any field not listed above, so there is no `**kwargs`-style escape hatch from JSON. Set default tags via the direct Python constructor instead.
