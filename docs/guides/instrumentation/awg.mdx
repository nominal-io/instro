...
`config` does not have to be a file path. It also accepts a plain dict or an already-built `AWGConfig` object, so when the configuration comes from somewhere other than a file, you can pass it straight to `InstroAWG` instead of writing it out to JSON and reading it back.

```python
awg = InstroAWG(config="bench_awg.json", test_run="run_42")
```
