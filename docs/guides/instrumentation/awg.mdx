...
| `is_modulation_enabled(channel=N)` | `ch{N}.modulation_enabled` | telemetry |
...
| `is_modulation_enabled(channel)` | Read back whether modulation is enabled on channel |
...
- **`is_modulation_enabled(channel)`**: Read back whether modulation is enabled from the instrument.
