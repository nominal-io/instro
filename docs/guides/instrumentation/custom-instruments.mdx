...
We'll fake that lag in one line: each tick, the current temperature moves 20% of the way toward the setpoint, plus a small amount of noise. That's enough for the `set_target_temperature` command to have a dramatic, observable effect — you'll watch the temperature climb from room temperature toward 50°C, then toward 75°C, in real time.

### 1. Subclass `Instrument`

Every custom instrument is a subclass of `Instrument`. The base class owns the publisher list, the background-daemon thread, default tags, and the channel buffer — all things your subclass gets for free.
...
Override `open()` and `close()` to manage the connection to your device. Our simulated controller has nothing to open, so both overrides just call `super()` — but this is the seam where a real instrument would call `socket.create_connection`, open a VISA session, claim a USB interface, etc. The base `close()` already stops the background daemon and closes attached publishers, so user-side overrides only need to add their own teardown.
...
Every call to `read_temperature()` advances the simulated temperature one step and publishes the new value under `{name}.temperature_c`. With the constructor we wrote above, the background daemon calls this on its own every 500 ms — that's where the visible "warming up" comes from. Any publisher you attach (e.g. `NominalCorePublisher`) receives every sample.

### 3. Publish a `Command` with `@publish_command`

Commands are for *changing* instrument state — setpoints, mode switches, configuration changes. A `@publish_command`-decorated method returns a `Command` and the decorator publishes it. We use the parallel helper `_package_command(channel, value, timestamp, **tags)` to build one. Pass the descriptor *with* a trailing `.cmd` so the published channel name has it.
