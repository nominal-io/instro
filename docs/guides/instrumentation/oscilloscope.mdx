...
The three optional initial-state blocks are applied through the public setters when `open()` runs, so they publish the same `.cmd` channels the equivalent manual calls would. Blocks apply in order: `channels`, then `acquisition`, then `trigger`. If any setter raises, the driver is closed and the error propagates.
