...
- **Vendor-installable drivers**: External libraries/drivers developed by instrument vendors that must be installed to your system operating system. These are necessary to install, regardless of using `instro`, in order to interact with the vendor hardware on the system in any way/environment.
