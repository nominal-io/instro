...
`StaticValue` programs a DC level rather than a shape with an independent offset, so `set_offset()` raises on a `StaticValue` channel. Construct a new `StaticValue` and call `set_waveform()` to change the level.
