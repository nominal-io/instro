...
The Modbus transports address Modbus TCP and RTU (serial) devices, exposing raw function-code ops plus a typed codec. Reach for one when the register map is fixed in code, or when you want a standalone client addressing registers by number. For a comparison against the other transports, see [Transports](/library/transports/overview).

For a config-driven device where the register map lives in a JSON file rather than driver code, use [`ModbusDevice`](/library/protocols/modbus) instead. `ModbusDevice` composes a Modbus transport and adds semantic access by register alias, scaling, validation, and background polling.
...
Hold the [transport lock](/library/transports/overview#atomic-multi-step-sequences) across several ops to keep them atomic, for example selecting a page or bank register and then reading from it:
...
The valid `unit_id` range is 0 to 255 on `ModbusTCPTransport` and 0 to 247 on `ModbusRTUTransport` (Modbus over Serial Line reserves 248-255, so a real RTU/ASCII slave will never have one of those addresses); `check_unit_id()` validates an address against the transport's range up front. This is the same shared-ownership mechanism described in [Transports: shared ownership](/library/transports/overview#shared-ownership), applied to one line instead of one device with two categories.

For config-driven devices, [`ModbusDevice`](/library/protocols/modbus) wraps this pattern: pass the same transport to several devices, each with its own `unit_id`.
...
| `open(holder=None)` | Connect. Idempotent. See [Lifecycle](/library/transports/overview#lifecycle). |
| `close(holder=None)` | Disconnect. See [Shared ownership](/library/transports/overview#shared-ownership). |
