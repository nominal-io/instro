...
- **[`VisaDriver`](/library/transports/visa)**: VISA-attached instruments over GPIB, USB-TMC, TCP/IP (SOCKET, VXI-11, HiSLIP), and RS-232/RS-485. The transport every shipped SCPI driver sits on.
- **[`ModbusTCPTransport`/`ModbusRTUTransport`](/library/transports/modbus)**: Modbus TCP and RTU, with raw function-code access, typed register encode and decode, and per-call `unit_id` addressing so multiple units share one line.
