...
Use protocol clients for industrial devices that do not use SCPI or VISA transports, such as PLCs, sensors, motor controllers, and environmental monitors. Data from those devices flows through the same [publisher system](/library/publishers) and background worker infrastructure as instrument driver data.

## Supported protocols

- **[Modbus](/library/protocols/modbus)**: Modbus TCP and RTU client with config-driven register access, automatic polling, and publisher support.
- **[EtherNet/IP](/library/protocols/ethernetip)**: Allen-Bradley PLC tag access with config-driven scalar reads, writes, and batched background collection.
