...
<Info>
  ...
  (`configure_digital_channel` becomes `configure_digital_input` /
  `configure_digital_output` / `configure_digital_port`).
</Info>
...
daq.configure_voltage_input("ai0", alias="ch_0", range_min=0, range_max=5)
daq.configure_digital_output("port0/line0", alias="do_0", logic=Logic.HIGH)

# After: fully qualified channel
daq = InstroDAQ(name="myDAQ", driver=NIDAQDriver(device_id="Dev1"))
daq.configure_voltage_input("Dev1/ai0", alias="ch_0", range_min=0, range_max=5)
daq.configure_digital_output("Dev1/port0/line0", alias="do_0", logic=Logic.HIGH)
```
...
`InstroDAQ.configure_digital_channel(...)` is gone. It used an optional `port_width` argument to decide at call time whether you were configuring a single line or a whole port. That one entry point is now split by shape, and lines carry their direction in the method name:

- `configure_digital_input(physical_channel, *, logic, logic_level=None, alias=None)` / `configure_digital_output(physical_channel, *, logic, logic_level=None, alias=None)` — a single digital line.
...
daq.configure_digital_output("Dev1/port0/line0", logic=Logic.HIGH, alias="do_0")
