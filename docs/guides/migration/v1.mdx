...
<Info>
  This is **v1** of the migration guide. It covers three changes that shipped in
  ...
  on each instrument to preserve the pre-v1.0 wire format), and
  [NI physical channel naming](#ni-physical-channel-naming) (NI DAQmx channels
  must now be fully qualified, with no compatibility flag).
</Info>
...
daq.configure_digital_channel(direction=Direction.OUTPUT, physical_channel="port0/line0", alias="do_0", logic=Logic.HIGH)
...
daq.configure_digital_channel(direction=Direction.OUTPUT, physical_channel="Dev1/port0/line0", alias="do_0", logic=Logic.HIGH)
```
