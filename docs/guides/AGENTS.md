...
6. Point the driver page's SDK links at the driver's class page, `https://nominal-io.github.io/instro/instruments/generated/<dotted.path.to.DriverClass>/`. That page only exists once the driver is in the Vendor Drivers table and hidden autosummary list of `docs/sdk/instruments/<category>.md` (see [`docs/sdk/AGENTS.md`](../sdk/AGENTS.md#adding-a-vendor-driver)) — `docs/sdk` is a separate Sphinx site, not this one, but the link is dead without it. Verify with `just build-docs && just check-sdk-links`.
...
8. **`docs/sdk/`** (sibling Sphinx site) — create `docs/sdk/instruments/<category>.md` in the category page layout from [`docs/sdk/AGENTS.md`](../sdk/AGENTS.md#category-page-layout) and add it to the Instruments toctree in `docs/sdk/index.md`, so step 6 of "Adding a new instrument driver" has somewhere to link.
