...
```{literalinclude} ../../../examples/modbus/sample_device.json
:language: json
```
...
:::{tab-item} Constructor (recommended)
```python
device = ModbusDevice(
    "my_device.json",
    connection={"transport": "tcp", "host": "192.168.1.10", "port": 502},
...
.. autosummary::
   ...
   ~instro.modbus.types.ModbusConfig
   ~instro.modbus.types.TimingConfig
   ~instro.modbus.types.RegisterDef
   ~instro.modbus.types.BitDef
```
...
| {py:class}`~instro.lib.types.LinearScale` | {pysummary}`instro.lib.types.LinearScale` |
