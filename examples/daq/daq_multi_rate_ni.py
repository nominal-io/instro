...
with daq_fast, daq_slow:
    ...
    for i in range(CHANNELS_PER_TASK):
        daq_fast.configure_voltage_input(
            physical_channel=f"{FAST_MODULE}/ai{i}",
            alias=f"fast_channel{i}",
            range_min=-5,
            range_max=5,
        )
        daq_slow.configure_voltage_input(
    ...
    daq_fast.configure_ai_hw_sample_rate(sample_rate=FAST_SAMPLE_RATE)
    daq_slow.configure_ai_hw_sample_rate(sample_rate=SLOW_SAMPLE_RATE)
