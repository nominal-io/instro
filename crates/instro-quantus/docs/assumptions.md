...
| A21 | `/item/list` orders channels immediately after their parent module (adjacency). The client now hard-errors when a channel's ItemName differs from the preceding module's (channels carry the module name per vendor convention), so a violation is loud, not silent | low | Vendor C# builds the tree from `/system/settings` Children instead; consider switching if hardware trips the guard. |
