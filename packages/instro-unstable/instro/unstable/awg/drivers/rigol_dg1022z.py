...
class RigolDG1022Z(AWGDriverBase):
    ...
    def set_waveform(self, channel: int, waveform: Waveform) -> None:
        ...
        with self._visa.lock():
            # Switching to a non-sweepable waveform (Pulse, StaticValue) correctly auto-disables an
            # active sweep on the instrument. Switching to a sweep-compatible waveform does not: the
            # sweep keeps running over its old start/stop range while this write silently lands on a
            # dormant "fixed frequency" register, so get_waveform() would report a frequency that
            # isn't what's actually being output.
            if not isinstance(waveform, (Pulse, StaticValue)) and self._sweep_enabled(channel):
                raise ValueError(
                    f"the DG1022Z cannot set a new waveform on channel {channel} while a sweep is active on it"
                )
    ...
    def get_waveform(self, channel: int) -> Waveform:
        ...
        with self._visa.lock():
            ...
            fields = resp.split(",")
            name = fields[0]
            if name in ("SIN", "HARMONIC"):
                # A channel with harmonics enabled reports APPL? as "HARMONIC" instead of "SIN", but the
                # underlying carrier (and field layout) is still the programmed Sine.
