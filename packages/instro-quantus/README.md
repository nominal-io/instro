...
`_quantus` module (PyO3 over the `instro-quantus` crate: REST configuration
engine + binary stream engine that drains the device socket outside the GIL).
...
- `crates/instro-quantus` — reusable pure-Rust client crate (root Cargo
  workspace member)
- `packages/instro-quantus` — this maturin package: `QuantusDevice` (Python) +
  the `_quantus` PyO3 bindings (standalone `instro-quantus-py` crate, own
  Cargo.lock)
- `crates/instro-quantus-sim` — device simulator used by tests and
  `examples/quantus/` (root workspace member; `cargo run -p instro-quantus-sim`)
...
`crates/instro-quantus/{PLAN.md,docs/}`.
