...
REST_DUMPS = {
    ...
    "autozero_settings.json": "/autoZero/settings/",
...
def capture_items(base: str, item_list, out_dir: Path):
    ...
    for item in item_list:
        ...
        if item_id is None:
            continue
        name = str(item.get("ItemName", "unknown")).replace(" ", "_").replace("/", "_")
        for kind, path in (
            ("settings", f"/item/settings/?itemId={item_id}"),
            ("operationMode", f"/item/operationMode/?itemId={item_id}"),
        ):
            try:
                payload = fetch_json(base, path)
            except (urllib.error.URLError, OSError) as exc:
                print(f"  WARN {path}: {exc}")
                continue
            (items_dir / f"item_{item_id:03d}_{name}_{kind}.json").write_text(json.dumps(payload, indent=2))
