...
- **A driver sharing an injected connection passes itself as the holder: `open(self)` / `close(self, ...)`.** `open(self)` opens the connection if needed, registers this driver as an owner, and reports whether it is the first owner, so one-time device setup runs exactly once no matter how many drivers share the connection; `close(self, ...)` mirrors it on the way out, running `on_last_release` and tearing down only when the last owner leaves. Bare `open()`/`close()` (no holder) is the sole-owner path: it skips this accounting, and a bare `close()` declines with a warning while any holder remains.
- **A driver whose post-`open(self)` device setup raises must `close(self)` before propagating.** Otherwise the driver is left in `_holders` despite failing to open, so a retried `open(self)` reports not-first-owner and skips the setup it needs to redo.
