...
- **Shared JSON-config blocks live in `instro/lib/config.py`.** `TimingConfig`, the publisher config models (`NominalCorePublisherConfig`, `FilePublisherConfig`, `PublisherConfigType`), `build_publisher`, and the generic `load_config(config, model_cls)` loader are identical across category configs, so category `config.py` modules (`instro/psu/config.py`, `instro/dmm/config.py`) import them from there instead of redefining them. Category-specific blocks (the vendor registry, the driver config, DMM's `measurement` block) stay in the category's own `config.py`. Modbus and EtherNet/IP predate this and keep their own divergent `TimingConfig` variants in their own types modules.
